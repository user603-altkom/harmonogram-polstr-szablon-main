...
export default function Strona() {
  ...
  function dodajNadplate(): void {
    const bladWalidacji = bladNadplaty();
    if (bladWalidacji) { setBlad(bladWalidacji); return; }
    setBlad('');
    setNadplaty((poprzednie) => [...poprzednie, { miesiac: '', kwota: '', tryb: 'skroc_okres' }]);
  }
  function zamknijDialogNadplat(): void {
    const bladWalidacji = bladNadplaty();
    if (bladWalidacji) { setBlad(bladWalidacji); return; }
    setBlad('');
    setDialogNadplat(false);
  }
    ...
    <div className="ekran-v4">
      ...
      <main className="siatka-v4">
        <form className="panel-formularza" onSubmit={oblicz}>
          ...
          <section className="nadplaty-skrot" onClick={() => { setBlad(''); setDialogNadplat(true); }}><div><span className="etykieta-v4">Nadpłaty</span><span>{nadplaty.length ? `${nadplaty.length} zaplanowane` : 'Brak nadpłat'}</span></div><button type="button">{nadplaty.length ? 'Edytuj' : 'Dodaj'}</button></section>
